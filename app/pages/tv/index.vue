<template>
  <section class="mx-auto max-w-7xl px-4 py-8 sm:py-12 xl:px-0">
    <form class="mb-8 flex w-full flex-col justify-center gap-2 sm:mb-12 sm:flex-row" @submit.prevent="goToSearchResults">
      ...
      <input v-model="searchQuery" type="search" placeholder="Search TV shows" class="input input-bordered input-lg w-full" />
...
const getPopularTVShows = async () => {
  try {
    loading.value = true;
    const res: Results = await $fetch(`/api/tmdb/trending/tv?page=${page.value}`);
